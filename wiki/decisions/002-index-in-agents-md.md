...
3. 外置 index = 把导航中枢变懒加载，自毁"沉淀一次、每次可用"的前提。
...
## 相关
- 维护规则被细化：[ADR-007：拆分 index/log 触发](007-log-trigger-split.md)
- 互链约定：[ADR-008](008-interlink-with-markdown-links.md)
- 架构总览：[overview](../architecture/overview.md)
- 评估并放弃的 goals/loop 层（曾考虑在 AGENTS.md 加「当前 goal」指针区）：[ADR-009](009-no-goals-loop-layer.md)
- 摘要内容规范（无状态描述符）：[ADR-010](010-stable-descriptor-index.md)
